...
programs 1852, classes 46, states 597, terminal classes 1, indeterminate 62, divergence rate 0.0019, flow into polymorphic targets 1.97e-05
mean payoff 0.0481, efficient 2.0000, deadweight loss 1.9519, mean bits in support 2.80

| pi | state |
|---|---|
| 0.9732 | mono {Keep:1} |
| 0.0213 | mono {THEM(^Give):1} |

Transitions out of the support (share of mutation events, mutants responsible):

- mono {Keep:1}
    - 9.25e-05 -> mono {THEM(^Give):1}   via THEM(^Give) (9.25e-05, rho=1.02e-01 k*=100)
...
- mono {THEM(^Give):1}
    ...
    - 5.85e-04 -> mono {THEM(^Keep):1}   via THEM(^Keep) (5.85e-04, rho=6.43e-01 k*=100)
    - 3.56e-04 -> mono {THEM(^X):1}   via THEM(^X) (3.56e-04, rho=4.03e-01 k*=100)
    - 1.08e-05 -> mono {or(X,THEM(^Keep)):1}   via or(X,THEM(^Keep)) (1.08e-05, rho=4.07e-01 k*=100)
...
INDETERMINATE transitions (replicator did not converge): 62; first: from poly {X:0.5, THEM(THEM):0.25, and(X,THEM(^Give)):0.25} with mutant or(X,THEM(^Keep))

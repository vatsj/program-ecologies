### arm=weak, n=5, game=bos, N=100, w=1.0, x_on=True, role=True, mode=square, fmap=exp

programs 902, classes 54, states 386, terminal classes 1, indeterminate 0, divergence rate 0.0010, flow into polymorphic targets 1.11e-05
mean payoff 1.4928, efficient 1.5000, deadweight loss 0.0072, mean bits in support 3.30

| pi | state |
|---|---|
| 0.4776 | mono {B:1} |
| 0.4776 | mono {A:1} |
| 0.0206 | poly {not(ROLE):0.33, THEM(^A):0.67} |
| 0.0206 | poly {not(ROLE):0.33, THEM(^B):0.67} |
...
- poly {not(ROLE):0.33, THEM(^A):0.67}
    - 8.41e-05 -> mono {A:1}   via A (8.41e-05, rho=3.38e-04 k*=100)
    - 8.41e-05 -> mono {B:1}   via B (8.41e-05, rho=3.38e-04 k*=100)
    ...
    - 1.12e-08 -> mono {or(X,THEM(THEM)):1}   via or(X,THEM(THEM)) (1.12e-08, rho=3.08e-04 k*=100)
    - 1.12e-08 -> mono {or(X,THEM(ME)):1}   via or(X,THEM(ME)) (1.12e-08, rho=3.08e-04 k*=100)
- poly {not(ROLE):0.33, THEM(^B):0.67}
    ...
    - 1.12e-08 -> mono {and(X,THEM(THEM)):1}   via and(X,THEM(THEM)) (1.12e-08, rho=3.08e-04 k*=100)
    - 1.12e-08 -> mono {and(X,THEM(ME)):1}   via and(X,THEM(ME)) (1.12e-08, rho=3.08e-04 k*=100)

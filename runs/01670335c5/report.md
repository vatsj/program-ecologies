...
programs 902, classes 54, states 676, terminal classes 1, indeterminate 0, divergence rate 0.0010, flow into polymorphic targets 6.60e-03
mean payoff 1.1108, efficient 1.5000, deadweight loss 0.3892, mean bits in support 3.26

| pi | state |
|---|---|
| 0.4261 | mono {C:1} |
| 0.1575 | mono {D:1} |
| 0.1256 | poly {X:0.65, ROLE:0.34, not(ROLE):0.01} |
| 0.0965 | mono {X:1} |
| 0.0726 | mono {ROLE:1} |
| 0.0280 | poly {X:0.01, ROLE:0.66, not(ROLE):0.33} |
| 0.0159 | poly {X:0.67, ROLE:0.33} |
| 0.0158 | poly {C:0.5, D:0.49, X:0.01} |
| 0.0133 | mono {not(ROLE):1} |
| 0.0089 | poly {C:0.5, D:0.5} |
| 0.0054 | mono {or(X,ROLE):1} |
...
- poly {C:0.5, D:0.5}
    ...
    - 5.93e-03 -> poly {C:0.33, D:0.33, ROLE:0.34}   via ROLE (5.93e-03, rho=3.11e-02 k*=33)
    - 3.65e-04 -> mono {not(ROLE):1}   via not(ROLE) (3.65e-04, rho=7.69e-03 k*=100)
    - 7.15e-05 -> poly {C:0.34, D:0.33, THEM(THEM):0.33}   via THEM(THEM) (7.15e-05, rho=3.28e-02 k*=33)
- mono {or(X,ROLE):1}
    - 2.80e-03 -> mono {C:1}   via C (2.80e-03, rho=1.12e-02 k*=100)
    - 2.48e-03 -> mono {D:1}   via D (2.48e-03, rho=9.95e-03 k*=100)
    ...
    - 2.73e-04 -> poly {or(X,ROLE):0.92, THEM(THEM):0.08}   via THEM(THEM) (2.73e-04, rho=1.25e-01 k*=8)
- mono {and(X,ROLE):1}

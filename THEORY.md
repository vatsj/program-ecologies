...
3. **Standing variance.** Does the shadow survive $\varepsilon N \ge 1$, where the conditional program is paid for its conditionality on path? Experiment: agent-based Moran at $\varepsilon N \in \{0.1, 1, 10\}$ (RESULTS.md, "Standing variance").
...
5. **Island model.** Conjecture A: with migration between demes, a deme fixed at the conditional cooperator seeds others faster than a deme fixed at its shadow, because the shadow-fixed deme is re-invaded by defectors. Conjecture B′: the island stationary distribution puts the cooperator's share at a finite fraction as the number of demes grows, at fixed $\varepsilon N$ per deme.

...
**Revised claim.** Selection over source-observing programs reaches efficiency iff the efficient outcome is stable against its own unconditional shadow — which is the case when it is a strict Nash outcome of the `ROLE`-symmetrized base game, and not the case in dilemmas or in fixed-role bargaining. `ROLE` resolves the coordination obstacle (it supplies the correlation that Chicken, demand and BoS need) and nothing else.

**What would change it.** Spatial structure, which keeps the shadow and the conditional program from being on-path equivalent (a neighbour of a defector is not a neighbour of a cooperator); standing variance, i.e. finite $\varepsilon N$ where the shadow and the defector are present together and the conditional program is no longer on-path indistinguishable from its shadow (§5, §9); or a self-reference primitive without grounding cost.
